...
impl Plot {
    pub fn contains(&self, (x, y): (f64, f64)) -> bool {
        self.corner.0 <= x
            && x < self.corner.0 + self.width
            && self.corner.1 >= y
            && y > self.corner.1 - self.height
    }
    ...
    fn draw_str(&mut self, (r, c): (usize, usize), string: &str, max_len: usize) {
    ...
    pub fn good_cell_size(&self, divs: f64) -> (f64, f64) {
        let cell_size = |dim: f64| {
            ...
            let two_log = (2.0_f64).log10();
            let five_log = (5.0_f64).log10();
            let log_dim = n
                + (if frac > five_log {
                    five_log
                } else if frac > two_log {
                    two_log
                } else {
                    0.0
                });
            (10.0_f64).powf(log_dim)
        };
        (cell_size(self.width), cell_size(self.height))
    }

    pub fn draw_gridlines(&mut self, with_labels: bool, (cell_wid, cell_hei): (f64, f64)) {
        let xs = linspace(
            (self.corner.0 / cell_wid).ceil() * cell_wid,
            self.corner.0 + self.width,
            cell_wid,
        );
        let ys = linspace(
...
impl Plot {
        ...
        F: FnMut(f64) -> Result<f64, E>,
    {
        ...
        for c in 0..self.columns {
            ...
            if maxy <= self.corner.1 - self.height || miny >= self.corner.1 {
                continue;
            }
        ...
        F: FnMut(f64, f64) -> Result<f64, E>,
    {
        ...
        for r in 0..grid_rows {
            for c in 0..grid_cols {
                ...
                states[idx] = if let Ok(val) = f(self.col_to_x(c), self.row_to_y(r)) {
                    if val > 0.0 {
                        S::P
                    } else {
                        S::N
        ...
        for r in 0..self.rows {
            for c in 0..self.columns {
                ...
                let symb = match (
                    ...
                    (S::P, S::P, S::N, S::N) | (S::N, S::N, S::P, S::P) => '~',

                    (S::P, S::N, S::N, S::P) | (S::N, S::P, S::P, S::N) => '!',

                    (S::N, S::P, S::N, S::P) | (S::P, S::N, S::P, S::N) => '+',
                    _ => continue,
                };
                self[(r, c)] = symb;
            }
...
impl Plot {
    pub fn new(mut options: HashMap<String, Object>) -> Result<Plot, ErrObject> {
        ...
        if !keys.is_empty() {
            return Err(eval_err!("Unknown options '{:?}'", keys,));
        }

        let width = xmax - xmin;
        let height = ymax - ymin;
        if width <= 0.0 || height <= 0.0 {
            return Err(eval_err!("Width and Height of plot must be positive"));
        }
    ...
    pub fn draw_obj(&mut self, mut obj: Object) {
        ...
        match arity.ok_or_err().and_then(|a| a.cast()) {
            ...
            Ok(1) => self.plot(|x| call!(obj(x)).ok_or_err()?.cast()),
            Ok(2) => self.plot_impl(|x, y| call!(obj(x, y)).ok_or_err()?.cast()),
            _ => {}
        }
    }
}
...
impl AddAssign<Object> for Plot {
    fn add_assign(&mut self, rhs: Object) {
        self.draw_obj(rhs);
    }
}
...
impl Add<Object> for Plot {
    type Output = Plot;
    fn add(mut self, rhs: Object) -> Plot {
        self += rhs;
        self
    }
}
