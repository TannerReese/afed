            ...
            if let Inner::Var {name, is_defn, ..} = &self_imm.0[id].inner {
                if *is_defn { true } else {
                    ...
                    if let Inner::Var {target, ..} = &mut self.0[id].inner {
                        *target = tgt;
                    } else { unreachable!() }  // It has to be a variable node
...
impl ExprArena {
    ...
    fn from_bltn(
        ...
        Bltn::Map(elems) => {
            ...
            self.create_node(vec![], Inner::Map{target: None, elems})
        },
    }}
}
...
impl ExprArena {
    ...
    pub fn get_defns(&self, exp: ExprId) -> Vec<String> {
        self.0[exp].vars.iter().filter_map(|&id|
            if let Inner::Var {name, is_defn, ..} = &self.0[id].inner {
                if *is_defn { Some(name.clone()) } else { None }
            } else { panic!("VarId doesn't refer to a variable node") }
        ).collect()
    }
    ...
    pub fn create_map(&mut self, elems: Vec<ExprId>) -> ExprId {
        ...
        let mut target = None;
        for var_id in defns.into_iter() {
            if let Inner::Var {
                name, is_defn, target: Some(id)
            } = &mut self.0[var_id].inner { if *is_defn {
                // Optional map target is indicated using empty variable name
                if name == "" {
                    if target.is_none() {
                        target = Some(*id);
                    } else { panic!("Redefinition of map target"); }
        ...
        self.create_node(vars, Inner::Map {target, elems: entries})
    }


    pub fn create_var(&mut self, name: String) -> VarId {
        self.0.alloc_with_id(|id| Node {
            simplifying: Cell::new(false), vars: vec![id],
            saved: true, value: Cell::new(None),
            inner: Inner::Var {name, is_defn: false, target: None},
        })
    }
    ...
    pub fn create_defn(&mut self, name: String, body: ExprId) -> VarId {
        ...
        self.0.alloc_with_id(|id| {
            vars.push(id);
            Node {
                simplifying: Cell::new(false), vars,
                saved: true, value: Cell::new(None),
                inner: Inner::Var {name, is_defn: true, target: Some(body)},
            }
        })
    }
    ...
    pub fn create_call(&mut self,
        caller: ExprId, path: Vec<String>, args: Vec<ExprId>
    ) -> ExprId {
        if path.len() == 0 && args.len() == 0 { return caller }
    ...
    pub fn from_obj(&mut self, obj: Object) -> ExprId {
        let inner = if obj.is_a::<Map>() {
            ...
            Inner::Map {target: None, elems:
                elems.into_iter().map(|(key, child)|
...
impl ExprArena {
    ...
    fn access(&self,
        ...
        while let Some(key) = path.as_slice().get(0) {
            match &self.0[exp].inner {
                ...
                Inner::Var {target: Some(id), ..} => { exp = *id; },
                _ => break,
            }
        }
        exp
    ...
    fn simplify(&self, args_used: &mut Vec<ArgId>, exp: ExprId) -> bool {
        ...
        if node.simplifying.get() {
            ...
            return true;
        } else { node.simplifying.set(true) }
        ...
        let obj = match &node.inner {
            ...
            Inner::Map {target: None, elems} => {
                let elems = elems.clone();
                let mut cnst = true;
                for &id in elems.values() { cnst &= simplify!(id); }
            ...
            &Inner::Map {target: Some(id), ..} => if simplify!(id) {
                Some(self.take(id))
            } else { None },
            ...
            Inner::Func {pats, body, ..} => {
                ...
                if !cnst { None } else {
                    ...
                    if let Inner::Func{
                        ...
                        let pats = std::mem::replace(pats, vec![]);
                        Some(Func::new(name, pats, *body, arena))
                    } else { unreachable!() }
                }
            },
        };
